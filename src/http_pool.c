                   ...
                   int max_parallel_downloads)
{
    ...
    pool->share_handle = curl_share_init();
    if (!pool->share_handle) return RET_FAIL;
...
{
    if (!pool || !pool->is_initialized) return RET_FAIL;
    if (pool->share_handle) curl_share_cleanup(pool->share_handle);
    memset(pool, 0, sizeof(http_pool_t));
    return RET_SUCCESS;
}

/* ── http_pool_get_parallel ─────────────────────────────────────────────── */
RET http_pool_get_parallel(http_pool_t     *pool,
                           download_task_t *tasks,
                           int              num_tasks,
                           int              max_concurrent)
{
    ...
    for (int i = 0; i < num_tasks; i++) {
        ...
        curl_easy_setopt(eh, CURLOPT_URL,       tasks[i].url);
        curl_easy_setopt(eh, CURLOPT_WRITEDATA, (void *)tasks[i].data);
        /* store task index so we can retrieve it in the completion loop */
        curl_easy_setopt(eh, CURLOPT_PRIVATE,   (void *)(intptr_t)i);

#ifdef DEBUG_FIRST_TILE
        if (i == 0) curl_easy_setopt(eh, CURLOPT_VERBOSE, 1L);
#endif
    ...
    /* ── activity bitmap: 1 = handle still inside multi ─────────────────── */
    int *active = (int *)calloc(num_tasks, sizeof(int));
    if (!active) {
        for (int i = 0; i < num_tasks; i++) curl_easy_cleanup(tasks[i].easy_handle);
        curl_multi_cleanup(multi_handle);
        return RET_FAIL;
    }
    for (int i = 0; i < num_tasks; i++) active[i] = 1;
    ...
    do {
        ...
        /* ── Early Termination (Task 3 / CTS algorithm step 4) ──────────── *
         * Only active when tau > 0.  On each wakeup we scan every in-flight  *
         * task.  If *prob_ptr has fallen below tau we call                    *
         * curl_multi_remove_handle() which immediately sends:                 *
         *   HTTP/3  → RESET_STREAM   (stops UDP datagram processing, Δstime) *
         *   HTTP/2  → RST_STREAM     (frees stream slot)                     *
         *   HTTP/1.1→ closes TCP connection                                   *
         * This reclaims both Δstime (kernel UDP/TLS) and Δutime (decoding).  *
         * ──────────────────────────────────────────────────────────────────── */
        if (pool->early_term_tau > 0.0f) {
            for (int i = 0; i < num_tasks; i++) {
                if (!active[i] || !tasks[i].prob_ptr) continue;
        ...
        if (still_running) {
            mc = curl_multi_poll(multi_handle, NULL, 0, 1000, NULL);
            if (mc != CURLM_OK) {
                fprintf(stderr, "[http_pool] curl_multi_poll: %s\n",
                        curl_multi_strerror(mc));
                break;
            }
        }
